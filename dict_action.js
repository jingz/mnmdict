...
function transform_longdo_result (raw_html, word) {
  ...
  let filter_sources = ['NECTEC Lexitron Dictionary EN-TH', 'NECTEC Lexitron-2 Dictionary (TH-EN)']
...
function longdo_lookup(word, cb, bf, last_char, do_log) {
  ...
  if (is_english(word)) {
    chrome.runtime.sendMessage(null, {
      ...
      let breVoice = raw.find('span.brefile') // British
    ...
    if(tresult.data.length > 0) {
      ...
      let jointTask = setInterval(() => {
        if (longmanSuccessToken) {
          ...
          cb(tresult.data, soundlist, word, phonetic, raw_html);
